...
class QwenListingGenerator:
    ...
    def extract_expected_count(self, prompt: str) -> int:
        ...
        match = re.search(
            r"(\d+)\s+(?:ways|steps|items|things|points|stages|factors|methods)",
            prompt.lower(),
        )
        if match:
            return int(match.group(1))
    ...
    def clean_incomplete_content(self, content: str) -> str:
        ...
        incomplete_indicators = [
            ...
            re.search(
                r"\b(the|and|or|but|because|since|when|if|that|which|who|so|then|now|first|next|also)$",
                last_sentence,
                re.IGNORECASE,
            ),  # Ends with connecting word
            re.search(
                r"\b(I|we|he|she|it|they|this|that|there|here)\s*(am|is|are|was|were|will|would|should|could|might|need|want|think)?\s*$",
                last_sentence,
                re.IGNORECASE,
            ),  # Ends with incomplete subject-verb
            re.search(
                r"\b(to|for|from|with|by|in|on|at|of|about)\s*$",
                last_sentence,
                re.IGNORECASE,
    ...
    def generate_thinking_response(
        ...
        with timer(f"Full trace generation ({expected_count} items)", verbose=True):
            ...
            for turn in range(expected_count):
                ...
                with timer(
                    f"Turn {turn + 1}/{expected_count} generation", verbose=True
                ):
                    if turn == 0:
                    ...
                    print(
                        f"        📝 Raw thinking response length: {len(thinking_response)} chars"
                    )

                    # Fix incomplete thinking tag
                    if (
                        "<think>" in thinking_response
                        and "</think>" not in thinking_response
                    ):
                        # Extract content after <think> and clean it
                        think_start = thinking_response.find("<think>")
                        if think_start != -1:
                            content_after_think = thinking_response[
                                think_start + 7 :
                            ...
                            thinking_response = (
                                thinking_response[: think_start + 7]
                                + cleaned_content
                                + "</think>"
                            )
                        else:
                    ...
                    print(
                        f"        📝 Raw answer response length: {len(answer_response)} chars"
                    )

                    # Fix incomplete answer tag
                    if (
                        "<answer>" in answer_response
                        and "</answer>" not in answer_response
                    ):
                        # Extract content after <answer> and clean it
                        answer_start = answer_response.find("<answer>")
                        if answer_start != -1:
                            content_after_answer = answer_response[
                                answer_start + 8 :
                            ...
                            answer_response = (
                                answer_response[: answer_start + 8]
                                + cleaned_content
                                + "</answer>"
                            )
                        else:
                    ...
                    think_match = re.search(
                        r"<think>(.*?)</think>", thinking_response, re.DOTALL
                    )
                    thinking_content = (
                    ...
                    answer_match = re.search(
                        r"<answer>(.*?)</answer>", answer_response, re.DOTALL
                    )
                    answer_content = (
                    ...
                    if not thinking_content:
                        print(
                            f"        ⚠️  Warning: No thinking content extracted from response"
                        )
                    if not answer_content:
                        print(
                            f"        ⚠️  Warning: No answer content extracted from response"
                        )
                    ...
                    if (
                        thinking_content or answer_content
                        ...
                        print(
                            f"        ✅ Completed turn {turn + 1}: thinking={len(thinking_content)} chars (~{thinking_tokens} tokens), answer={len(answer_content)} chars (~{answer_tokens} tokens)"
                        )

                        # Warn if thinking exceeds expected limit
                        if thinking_tokens > 256:
                            print(
                                f"        ⚠️  Warning: Thinking section has ~{thinking_tokens} tokens, exceeds 256 token limit"
                            )
                    else:
                        print(
                            f"        ❌ Skipping turn {turn + 1}: no valid content extracted"
                        )
            ...
            print(
                f"    📊 Generation summary: {successful_turns}/{expected_count} turns completed successfully"
            )
            if successful_turns < expected_count:
                print(
                    f"    ⚠️  Warning: Expected {expected_count} items but only generated {successful_turns}"
                )
    ...
    def step1_generate_category_prompts(self, category: str) -> List[str]:
        ...
        response = self.generate_response(
            messages, max_new_tokens=4096, temperature=0.8
        )
    ...
    def step2_generate_interleaved_responses(
        ...
        for i, prompt in enumerate(
            tqdm(prompts, desc=f"Generating {category} responses")
        ):
            try:
                ...
                if i < 2:
                    ...
                    print(
                        f"Individual answers: {response_data.get('individual_answers', [])[:3]}..."
                    )  # Show first 3
                    print(
                        f"Combined answer preview:\n{response_data['answer'][:500]}..."
                    )
    ...
    def _infer_category(self, prompt: str) -> str:
        ...
        if any(
            ...
            for word in ["program", "code", "software", "api", "debug", "test"]
        ):
            return "programming"
        elif any(
            word in prompt_lower
            for word in ["business", "startup", "marketing", "entrepreneur"]
        ):
            return "business"
        elif any(
            word in prompt_lower
            for word in ["green card", "visa", "legal", "law", "court"]
        ):
            return "legal"
        elif any(
            word in prompt_lower for word in ["health", "medical", "doctor", "medicine"]
        ):
            return "health"
        elif any(
            word in prompt_lower
            for word in ["environment", "climate", "carbon", "green"]
        ):
            return "environmental"
        elif any(
            word in prompt_lower
            for word in ["security", "vulnerability", "cyber", "hack"]
        ):
            return "cybersecurity"
        elif any(
            word in prompt_lower
            for word in ["finance", "invest", "money", "stock", "financial"]
        ):
            return "finance"
        elif any(
            word in prompt_lower for word in ["education", "learn", "teach", "school"]
        ):
            return "education"
        elif any(
            word in prompt_lower for word in ["travel", "country", "geography", "place"]
        ):
            return "travel"
        else:
            return "general"
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate reasoning prompts and interleaved responses with Qwen"
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default="reasoning_data",
    ...
    if args.skip_step1 and args.prompts_file:
        ...
        prompts_file = os.path.join(
            args.output_dir, f"generated_{args.category}_prompts.txt"
        )
    ...
    with tqdm(
        total=total_samples, desc=f"Generating {args.category} responses"
    ) as pbar:
        for i, prompt in enumerate(prompts):
            for sample_idx in range(args.samples_per_prompt):
                try:
                    ...
                    with timer(
                        f"Complete interleaved trace for sample {sample_id}",
                        verbose=True,
                    ):
                        response_data = generator.generate_thinking_response(
                            prompt, max_new_tokens_per_turn=args.max_tokens_per_turn
                        )
                        ...
                        result = {
                            ...
                            "messages": [
                                {
                                    "role": "system",
                                    "content": INTERLEAVED_SYSTEM_PROMPT,
                                },
                                {"role": "user", "content": prompt},
                            ],
                            "generated_thinking": response_data["thinking"],
                            ...
                            "turns": response_data.get("turns", []),
                            "individual_thinking": response_data.get(
                                "individual_thinking", []
                            ),
                            "individual_answers": response_data.get(
                                "individual_answers", []
                            ),
                        }
                        ...
                        print(
                            f"    ✅ Successfully generated sample {sample_id} with {answer_count} answer tags"
                        )

                        # Print each generated trace to see what it looks like
                        print(f"\n{'=' * 80}")
                        print(
                            f"SAMPLE {sample_id}: {args.category.upper()} INTERLEAVED TRACE"
                        )
                        print(f"{'=' * 80}")
                        print(f"PROMPT: {prompt}")
                    ...
                    print(
                        f"❌ Error processing {args.category} prompt {i} sample {sample_idx}: {str(e)}"
                    )
                    pbar.update(1)
                    continue
    ...
    print(
        f"    Total generation time: {total_generation_time:.1f}s ({total_generation_time / 60:.1f} minutes)"
    )
    ...
    if results:
        ...
        avg_time_per_tag = (
            total_generation_time / total_answer_tags if total_answer_tags > 0 else 0
        )
    ...
    with timer("Data preparation and cleaning", verbose=True):
        ...
        for result in results:
            ...
            sft_entry = {"question": result["prompt"], "answer": interleaved_trace}
    ...
    with timer(f"Data saving ({args.save_format} format)", verbose=True):
        if args.save_format == "parquet":
            ...
            train_path, test_path = save_parquet_with_split(
                sft_data, args.output_dir, filename_prefix, args.test_size
            )
            output_files = [train_path, test_path]
        else:
            # Legacy JSONL format
            output_file = os.path.join(
                args.output_dir, f"interleaved_{args.category}_dataset.jsonl"
            )
