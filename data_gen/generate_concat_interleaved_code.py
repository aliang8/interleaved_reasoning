...
def generate_concat_dataset(
    ...
    for i in tqdm(range(num_samples), desc="Generating paired coding traces"):
        ...
        reward_model = StandardizedRewardModel(
            ...
            style="code",
            unit_tests=[
                trace_data["reward_model"]["test_1"],
                trace_data["reward_model"]["test_2"],
            ],
            libs=[
                trace_data["reward_model"]["libs_1"],
                trace_data["reward_model"]["libs_2"],
            ],
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate paired interleaved coding traces for MBPP"
    )
    parser.add_argument(
        "--model_name",
        type=str,
    ...
    parser.add_argument(
        "--use_canonical",
        action="store_true",
        help="Use canonical solutions instead of generating new code solutions",
    )
