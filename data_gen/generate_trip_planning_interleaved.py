...
Think about: overall trip structure, main destinations, key logistical considerations, and general timeline. Don't get into daily details yet - just the big picture planning."""
            ...
            think_messages = messages + [
                {"role": "user", "content": overview_thinking_prompt}
            ]
            ...
            thinking_content = (
                thinking_response.replace("<think>", "").replace("</think>", "").strip()
            )
        ...
        with timer("Generate overview answer"):
            ...
            updated_messages = think_messages + [
                {
                    "role": "assistant",
                    "content": f"<think>\n{thinking_content}\n</think>",
                }
            ...
            answer_messages = updated_messages + [
                {
                    "role": "user",
                    "content": "Now provide a concise trip overview in <answer></answer> tags based on your thinking. Format it as a list of days with activities and timings:",
                }
            ]
            ...
            answer_content = (
                answer_response.replace("<answer>", "").replace("</answer>", "").strip()
            )
            ...
            # Add to conversation
            conversation.append({"role": "user", "content": overview_thinking_prompt})
            conversation.append(
                {
                    "role": "assistant",
            ...
            interleaved_parts.append(
                {"type": "thinking", "content": thinking_content, "turn": "overview"}
            )
            interleaved_parts.append(
                {"type": "answer", "content": answer_content, "turn": "overview"}
            )

        # Update base messages for subsequent days
        base_messages = messages + [
            {"role": "assistant", "content": full_overview_response}
        ]
...
Make this day practical and well-coordinated with the overall trip plan."""
                ...
                day_think_messages = base_messages + [
                    {"role": "user", "content": day_thinking_prompt}
                ]
                ...
                day_thinking_content = (
                    day_thinking_response.replace("<think>", "")
                    .replace("</think>", "")
                    .strip()
                )
                day_thinking_content = self.clean_incomplete_content(
                    day_thinking_content
                )

                print(
                    f"          Day {day} thinking: {len(day_thinking_content)} chars"
                )

            with timer(f"Generate day {day} answer"):
                # Add thinking to context
                day_updated_messages = day_think_messages + [
                    {
                        "role": "assistant",
                        "content": f"<think>\n{day_thinking_content}\n</think>",
                    }
                ...
                day_answer_messages = day_updated_messages + [
                    {
                        "role": "user",
                        "content": f"Now provide a detailed plan for Day {day} in <answer></answer> tags based on your thinking:",
                    }
                ]
                ...
                # Clean answer content
                day_answer_content = (
                    day_answer_response.replace("<answer>", "")
                    .replace("</answer>", "")
                    .strip()
                )
                ...
                # Add to conversation
                conversation.append({"role": "user", "content": day_thinking_prompt})
                conversation.append(
                    {
                        "role": "assistant",
                ...
                base_messages.append({"role": "user", "content": day_thinking_prompt})
                base_messages.append(
                    {"role": "assistant", "content": full_day_response}
                )
    ...
    def generate_trip_planning_prompts(self) -> List[str]:
        ...
        response = self.generate_response(
            messages, max_new_tokens=1024, temperature=0.8, enable_thinking=False
        )
        ...
        for line in lines:
            ...
            if len(cleaned_line) > 50 and any(
                word in cleaned_line.lower()
                for word in ["trip", "travel", "itinerary", "vacation", "visit", "plan"]
            ):
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate trip planning interleaved conversations"
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default="trip_planning_data",
    ...
    parser.add_argument(
        "--device_map", type=str, default="auto", help="Device map for model loading"
    )
    parser.add_argument(
        "--max_prompts",
        type=int,
    ...
    generator = TripPlanningGenerator(model_name=args.model, device_map=args.device_map)
    ...
    if args.skip_prompt_generation and args.prompts_file:
        ...
        prompts_file = os.path.join(
            args.output_dir, "generated_trip_planning_prompts.txt"
        )
    ...
    for i, prompt in enumerate(
        tqdm(prompts, desc="Generating trip planning conversations")
    ):
        try:
            with timer(f"Prompt {i + 1}/{len(prompts)}: '{prompt[:50]}...'"):
                conversation_data = generator.generate_trip_planning_conversation(
                    prompt
                )
                ...
                if i < 2:
                    ...
                    print(
                        f"Total interleaved parts: {len(conversation_data['interleaved_parts'])}"
                    )
                    ...
                    if parts:
                        ...
                        for j, part in enumerate(parts[:6]):  # Show first 6 parts
                            print(
                                f"  Part {j + 1}: {part['type']} ({part['turn']}) - {len(part['content'])} chars"
                            )
                        if len(parts) > 6:
                    ...
                    if len(sample_conv) > 3:
                        ...
                        for k in range(2, min(6, len(sample_conv)), 2):
                            if k < len(sample_conv) and k + 1 < len(sample_conv):
                                ...
                                print(
                                    f"  Assistant thinking: {len(assistant_turn.get('thinking', ''))} chars"
                                )
                                print(
                                    f"  Assistant answer: {len(assistant_turn.get('answer', ''))} chars"
                                )
    ...
    output_file_jsonl = os.path.join(
        args.output_dir, "trip_planning_interleaved_dataset.jsonl"
    )
    output_file_parquet = os.path.join(
        args.output_dir, "trip_planning_interleaved_dataset.parquet"
    )
    ...
    if results:
        ...
        for i, turn in enumerate(sample["conversation"]):
            if turn["role"] == "assistant":
                ...
                answer_len = len(turn.get("answer", ""))
                print(
                    f"  Turn {i // 2 + 1} ({turn_type}): {thinking_len} chars thinking, {answer_len} chars answer"
                )
    ...
    if results:
        ...
        if results:
            ...
            answer_preview = (
                parquet_example["answer"][:200] + "..."
                if len(parquet_example["answer"]) > 200
                else parquet_example["answer"]
            )
            parquet_preview = {
...
""")
    ...
    if results:
        ...
        print(
            f"   - Single interleaved string in 'answer' field with <think>/<answer> tags"
        )
