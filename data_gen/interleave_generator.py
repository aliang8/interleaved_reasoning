...
class InterleavedResponsesGenerator:
    ...
    def generate_thoughts(self, messages: List[Dict[str, str]], 
        ...
        if '<think>' in response and '</think>' not in response:
            ...
            if think_start != -1:
                ...
                response = response[:think_start + 7] + cleaned_content + "</think>"
            else:
                response += "</think>"
        
        return response

    def generate_answer(self, messages: List[Dict[str, str]], 
                       max_new_tokens: int = 512,
                       temperature: float = 0.2,
                       top_p: float = 0.7,
                       previous_solutions: List[str] = None) -> str:
        ...
        if '<answer>' in response and '</answer>' not in response:
            ...
            if answer_start != -1:
                ...
                response = response[:answer_start + 8] + cleaned_content + "</answer>"
            else:
                response += "</answer>"
