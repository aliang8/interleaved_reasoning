...
def process_mbpp_combined(local_dir, n=2, sep=" ", answer_sep="\n\n", prompt_combine_mode="space", llm_model_name="Qwen/Qwen3-8B", llm_device_map="auto", prompt_prefix=None):
    ...
    def combine_dataset(dataset, split):
        ...
        for i in range(0, len(dataset), n):
            ...
            instr = "\n\nYou should write self-contained code starting with:\n```\ndef task_func(args):\n```"
            group_dict["prompt"] = [p + instr for p in group_dict["prompt"]]
            # Combine unit tests for all problems in the group
            combined_unit_tests = [test_list_to_unittest(tests, func_name="task_func") for tests in group_dict["test_list"]]
    ...
    for dataset_config in config['datasets']:
        ...
        if dataset_name == "knights_and_knaves":
            ...
            if combine_n > 1:
                ...
                all_datasets[f"{dataset_name}_combined_{prompt_combine_mode}_{combine_n}"] = (None, test_dataset)
            else:
            ...
            if combine_n > 1:
                ...
                all_datasets[f"{dataset_name}_combined_{combine_n}"] = (train_dataset, test_dataset)
            else:
...
def main():
    ...
    parser.add_argument("--config_file", type=str, required=True, help="YAML config file specifying datasets to process")
