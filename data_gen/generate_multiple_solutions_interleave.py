...
def simple_generate(
    ...
    model_inputs = generator.tokenizer([text], return_tensors="pt").to(
...
def evaluate_single_interleaved_entry(
    result: Dict[str, Any],
    generator: InterleavedResponsesGenerator,
...
def to_single_solution_prompt_llm(
    prompt: str, generator: InterleavedResponsesGenerator, max_new_tokens=128
) -> str:
    full_prompt = f"{MULTIPLE_SOLUTIONS_PROMPTS['single_solution']}\n\nPrompt: {prompt}\n\nOutput: "
    return simple_generate(full_prompt, generator, max_new_tokens)


def split_if_specific_ways(
    prompt: str, generator: InterleavedResponsesGenerator, max_new_tokens=256
) -> tuple[Optional[list], Optional[str]]:
    full_prompt = (
        f"{MULTIPLE_SOLUTIONS_PROMPTS['split_prompts']}\n\nPrompt: {prompt}\n\nOutput: "
    )
    response = simple_generate(full_prompt, generator, max_new_tokens)
    return parse_numbered_list(response), response


def reword_split_prompt(
    split_prompt: str,
    original_prompt: str,
    generator: InterleavedResponsesGenerator,
    max_new_tokens=128,
...
def generate_code_and_thoughts(
    prompt,
    previous_solutions,
    generator: InterleavedResponsesGenerator,
    max_new_tokens=512,
...
def generate_function_signature(
    prompt: str, generator: InterleavedResponsesGenerator, max_new_tokens=64
) -> str:
    full_prompt = (
        f"{MULTIPLE_SOLUTIONS_PROMPTS['function_signature']}\n\nPrompt: {prompt}"
    )
    response = simple_generate(full_prompt, generator, max_new_tokens, do_sample=False)
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate interleaved code reasoning dataset from prompts"
    )
    parser.add_argument(
        "--prompt_file",
        type=str,
        default="data_gen/code_list_prompts.txt",
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="Number of code solutions per prompt (if available)",
    )
    parser.add_argument(
        "--code_model",
        type=str,
    ...
    generator = InterleavedResponsesGenerator(
        model_name=args.code_model, device_map=args.device_map
    )

    interleaved_results = []

    # Initialize code evaluator once
    config = OmegaConf.create({"max_concurrent": 4, "execute_sequential": False})
    ...
    for i, prompt in enumerate(tqdm(prompts, desc="Processing prompts")):
        ...
        if split_prompts:
            print(
                f"Detected specific approaches, split into {len(split_prompts)} prompts."
            )
            ...
            for j, split_prompt in enumerate(split_prompts):
                ...
                thought, code, code_response = generate_code_and_thoughts(
                    reworded_prompt, [], generator
                )
            ...
            for j in range(args.num_code_solutions):
                if j == 0:
                    ...
                    context_prompt = (
                        ...
                        + "\n\n".join(
                            [
                                f"Solution {k + 1}:\n{code}"
                                for k, code in enumerate(previous_codes)
                            ]
                        )
                ...
                thought, code, code_response = generate_code_and_thoughts(
                    context_prompt, previous_codes, generator
                )
                print(f"  Thought: {thought[:100]}...")
                code_blocks = extract_code_blocks(code_response)
                code_to_save = code_blocks[0] if code_blocks else code
        ...
        all_solutions_code = "\n\n".join(
            f"# Solution {idx + 1}:\n{sol['code']}"
            for idx, sol in enumerate(code_solutions)
        )
        ...
        if evaluate_single_interleaved_entry(
            interleaved_results[-1], generator, code_evaluator
        ):
    ...
    save_jsonl(
        interleaved_results, os.path.join(args.output_dir, f"{filename_prefix}.jsonl")
    )
    print(
