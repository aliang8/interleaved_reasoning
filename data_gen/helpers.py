...
class StandardizedRewardModel:
    ...
    def __init__(
        self, ground_truth, style="rule", unit_tests=None, libs=None, **kwargs
    ):
...
def save_to_parquet_all(all_datasets, local_dir, output_filename):
    ...
    for dataset_name, (train_data, test_data) in all_datasets.items():
        if train_data is not None:
            ...
            save_jsonl(
                train_data, os.path.join(local_dir, f"{dataset_name}_train.jsonl")
            )
...
def merge_prompts_with_llm(prompts, model_name=None, device_map=None):
    ...
    if (
        ...
        or model_name != LLM_MODEL_NAME
        or device_map != LLM_DEVICE_MAP
    ):
    ...
    user_prompt = (
        "Combine the following questions into a single, natural question that asks for all the information together.\n\n"
...
def combine_examples(
    ...
    if prompt_combine_mode == "numbered_list" and len(prompts) > 1:
        ...
        combined_prompt = merge_prompts_with_llm(
            prompts, model_name=llm_model_name, device_map=llm_device_map
        )
    else:
